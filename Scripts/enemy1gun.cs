...
public class enemy1gun : MonoBehaviour {
    ...
    void Update() {
        ...
        if(GetComponent<Enemy1Movement>().player.GetComponent<Gun>().health>0)
        {
    ...
    {
        if((GetComponent<Enemy1Movement>().startplayerShoot == 0))
        {
            firststartshoot = 0;
        }
        else if((GetComponent<Enemy1Movement>().startplayerShoot == 1) && firststartshoot==0)
        {
